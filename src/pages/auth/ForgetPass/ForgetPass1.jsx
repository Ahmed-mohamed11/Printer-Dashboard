...
const ForgetPassword = () => {
  ...
  const handleForgetPasswordStep1 = async () => {
    try {
      ...
      ErrorAlert({ text: error.response?.data?.message || error.response?.data?.errors[0]?.msg || "Error in sending code" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleForgetPasswordStep2 = async () => {
    try {
      ...
      ErrorAlert({
        text: error.response?.data?.message || "Wrong verification code"
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleForgetPasswordStep3 = async () => {
    try {
      ...
      ErrorAlert({ text: error.response?.data?.message || error.response?.data?.errors[0]?.msg || "An error occurred while resetting password" });
    } finally {
      setLoading(false);
    }
  };
